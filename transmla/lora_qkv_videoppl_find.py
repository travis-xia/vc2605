...
def low_rank_qkv_videoppl_find(model, tokenizer, train_loader, test_loader, ppl_tolerance=0.05, **kwargs):
    """
    遍历每一层的不同 kv_lora_rank 设置，测试对视频 ppl 的影响
    测试的 rank 值：使用全局配置 TEST_RANKS + BASELINE_RANK
    优化：基准只测试一次，然后每层只测试 TEST_RANKS 中的值
    目标：找到在可接受 PPL 损失范围内（默认 0.05）的最小 rank
    """
    
    print("\n" + "="*80)
    print("开始搜索每层 kv_lora_rank 对视频 PPL 的影响".center(80))
    ...
    if test_loader:
        ...
        baseline_ppl = evaluate_video_ppl(model, tokenizer, test_loader, message)
        print(f"基准配置视频 PPL: {baseline_ppl:.4f}")
    else:
        print("基准配置: 无测试数据")
    ...
    for layer_idx in range(num_layers):
        ...
        for rank in test_ranks:
            ...
            if test_loader:
                ...
                print(f"    Layer {layer_idx}, kv_lora_rank={rank}: 视频 PPL = {dataset_ppl:.4f}")
            else:
    ...
    # 打印汇总结果
    print("\n" + "="*80)
    print("搜索结果汇总 (VideoChat-Flash)".center(80))
    print("="*80)
    baseline_ppl_str = f"{baseline_ppl:.4f}" if baseline_ppl is not None else "N/A"
    print(f"基准配置视频 PPL (所有层 rank={BASELINE_RANK}): {baseline_ppl_str}")
...
def low_rank_qkv_videoppl(model, tokenizer, train_loader, test_loader, **kwargs):
    """
    原始的 low_rank_qkv_videoppl 函数，保持兼容性
    """
    message = "Calibrating rope-removed model's qkv outputs"
    rm_rope_qkv_outputs = get_qkv_calibrate_outputs(model, train_loader, message)
    ...
    if test_loader:
        message = "Evaluating lora-qkv model's video ppl"
        dataset_ppl = evaluate_video_ppl(model, tokenizer, test_loader, message)
        print(f'Low rank approximate QKV 视频 ppl: {dataset_ppl:.4f}')
