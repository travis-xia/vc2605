...
def main(args):
    ...
    # 生成每层的 kv_lora_rank 列表
    num_layers = len(model.model.layers)
    if not KV_LORA_RANKS:
        print(f"KV_LORA_RANKS 未设置，默认为每层 512（共 {num_layers} 层）。")
        kv_lora_rank_list = [512] * num_layers
    else:
        if len(KV_LORA_RANKS) != num_layers:
            raise ValueError(f"KV_LORA_RANKS 长度 {len(KV_LORA_RANKS)} 与模型层数 {num_layers} 不匹配")
        kv_lora_rank_list = KV_LORA_RANKS

    model = low_rank_qkv_videoppl(
        model,
        tokenizer,
        train_loader,
        test_loader,
        **{
            **vars(args),
            "kv_lora_rank_list": kv_lora_rank_list,
        },
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freqfold", type=str, default="8", help="Freqfold for removing RoPE, int or auto")
